...
*基于语句的复制*：<font color=red>在主服务器上执行的SQL语句，在从服务器上执行同样的语句。</font>MySQL默认采用基于语句的复制，效率比较高。一旦发现没法精确复制时，   会自动选着基于行的复制。

*基于行的复制*：<font color=red>把改变的内容复制过去，而不是把命令在从服务器上执行一遍. 从mysql5.0开始支持</font>

*混合类型的复制*:<font color=red>默认采用基于语句的复制，一旦发现基于语句的无法精确的复制时，就会采用基于行的复制。</font>
